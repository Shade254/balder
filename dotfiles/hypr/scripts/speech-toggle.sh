#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════════
# Speech-to-Text Toggle (Press Once to Start, Press Again to Stop)
# Single keybind that toggles recording on/off
# ═══════════════════════════════════════════════════════════════════════════

SOCKET_PATH="/tmp/speech-to-text.sock"
STATE_FILE="/tmp/speech-recording-state"
...
NOTIFICATION_ID=9876  # Fixed ID for replacing notifications
...
# Check if currently recording (state file exists)
...
")
    ...
    notify-send "🎙️ Recording..." "Press CMD+M again to stop and transcribe" \
        -t 0 -u normal -r "$NOTIFICATION_ID" &
...
")
    ...
    if [[ "$RESPONSE" == "RECORDING_STARTED" ]]; then
        # Create state file to track recording state
        touch "$STATE_FILE"
    else
        notify-send "🎤 Speech-to-Text" "❌ Failed to start: $RESPONSE" \
            -t 3000 -u critical -r "$NOTIFICATION_ID"
        exit 1
    fi
fi
